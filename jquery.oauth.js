...
(function(window, document, $, undefined) {
	...
	function extendOptions(options) {
		options = $.extend({
			type : 'GET',
			consumerKey : 'a9DtHbJ5UyLUua4umVtSAgQzwpd1ub8aeEBr2zhg',
			consumerSecret : 'sQbsWMHDMdhz9rMMlkJ3OtcoUnx91TuXiG5dDIKg',
			tokenSecret : '',
			url : ''
		}, options);
		...
		if (options.url.indexOf(':') == -1) {
			if (options.url.substr(0, 1) == '/') {
				...
				options.url = location.href.substr(0, location.href
						.lastIndexOf('/') + 1)
						+ options.url;
			}
		}
		...
		var baseStr = OAuth.decodeForm(OAuth.SignatureMethod
				.getBaseString(message));

		if (options.type === "DELETE") {

			//
			// Add Authorization header to the request.
			// We don't use parameters to send oAuth information as DELETE
			// requests
